...
class DetailTeamPresenterTest {
    ...
    private lateinit var mPresenter: DetailTeamContract.Presenter
    ...
    fun setUp() {
        ...
        favoriteRepository = Mockito.spy(TeamFavoriteRepository(ApplicationProvider.getApplicationContext()))
        scheduler = TestSchedulerProvider()
        mPresenter = DetailTeamPresenter(view, repository, favoriteRepository, scheduler)
    ...
    @Test
    fun `add favorite team, when data created, should update view`() {

        val insertedId = 1L
        val longFlowable = Flowable.just(insertedId)

        `when`(favoriteRepository.createTeamFavorite(
        ...
        )).thenReturn(longFlowable)

        mPresenter.addFavoriteTeam(
            teamId = team.idTeam.toString(),
            teamName = team.strTeam.toString(),
            teamBadge = team.strTeamBadge.toString(),
            teamManager = team.strManager.toString(),
        ...
        verify(view).onFavoriteTeamAdded(true)
    }

    @Test
    fun `remove favorite team, when data deleted, should update view`() {

        val booleanFlowable = Flowable.just(true)

        `when`(favoriteRepository.deleteTeamFavorite(idTeam)).thenReturn(booleanFlowable)

        mPresenter.removeFavoriteTeam(idTeam)

        verify(view).onFavoriteTeamRemoved(true)
    }

    @Test
    fun `is favorite, when data available on favorite, should update view`() {

        val teamFlowable = Flowable.just(team)

        `when`(favoriteRepository.getTeamFavorite(idTeam)).thenReturn(teamFlowable)

        mPresenter.isFavorite(idTeam)

        verify(view).setFavorite(true)
    }

    @Test
    fun `get a team, when data available, should update view`() {

        teamMutableList.addAll(teamList)
        teamResponse = TeamResponse(teamMutableList)
        teamResponseFlowable = Flowable.just(teamResponse)

        `when`(repository.getTeam(idTeam)).thenReturn(teamResponseFlowable)

        mPresenter.getTeam(idTeam)

        verify(view).displayTeam(team)
    }
}