...
class TeamRepositoryTest {
    ...
    private lateinit var service: TheSportDbService
    ...
    fun setUp() {
        ...
        teamResponse = TeamResponse(teams = teamList)

        //setup service with mock web server
        ...
        val retrofit = Retrofit.Builder()
            ...
            .build()
        service = Mockito.spy(retrofit.create(TheSportDbService::class.java))
        repository = TeamRepository(service)
    }

    @Test
    fun `get all team, when data available, should return result`() {

        val mockReponse = MockResponse()
            .setResponseCode(200)
            .setBody(gson.toJson(teamResponse))

        webServer.enqueue(mockReponse)

        val getAllTeam = repository.getAllTeam(idLeague)

        val testSubscriber = TestSubscriber<TeamResponse>()
        getAllTeam.subscribe(testSubscriber)

        testSubscriber.assertNoErrors()
        val result = testSubscriber.values()[0].teams?: Collections.emptyList()
        assertThat(result.size,  `is`(Matchers.greaterThan(0)))

    }

    @Test
    fun `get all team, when data not available, should return empty result`() {

        val emptyList : List<Team> = listOf()

        val mockReponse = MockResponse()
            .setResponseCode(200)
            .setBody(gson.toJson(TeamResponse(teams = emptyList)))

        webServer.enqueue(mockReponse)

        val getAllTeam = repository.getAllTeam("0000")
    ...
    @Test
    fun `get all team, when no league id given, should return empty result`() {
        ...
        val getAllTeam = repository.getAllTeam("")
    ...
    @Test
    fun `get a team, when data available, should return result`() {
        ...
        val getTeam = repository.getTeam(idTeam)

        val testSubscriber = TestSubscriber<TeamResponse>()
        getTeam.subscribe(testSubscriber)
    ...
    @Test
    fun `get a team, when data not available, should return empty result`() {
        ...
        val getTeam = repository.getTeam("000000")
    ...
    @Test
    fun `get a team, when no team id given, should return empty result`() {
        ...
        val getTeam = repository.getTeam("")
